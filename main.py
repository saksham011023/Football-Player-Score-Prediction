...
def transform_and_process_data(df):
    ...
    initial_transformer = joblib.load('pipelines/30_08_2024-15_58_22_custom_attribute_object.pkl')
    df = initial_transformer.transform(df)    

    transformer = joblib.load('pipelines/30_08_2024-15_58_22_full_transformer.pkl')
    transformed_data = transformer.transform(df)

    model = joblib.load('models/30_08_2024-16_25_57_best_model.pkl')
    predicted_value = model.predict(transformed_data)

    # print('Predicted Value : ', predicted_value)
    
    target_transformer = joblib.load('pipelines/30_08_2024-15_58_22_target_pipeline.pkl')
